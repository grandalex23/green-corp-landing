...
class Bubble {
   ...
   init() {
      this.color = COLORS[Math.floor(Math.random() * COLORS.length)] /* свойство color инициализируется случайным цветом из массива COLORS,
      у Math.random такой синтаксисис от 0 до (*) Числа*/
      this.size = generateDecimalBetween(1, 3); /*иниц-я. св-ва size класса случайным числом от 1 до 3. */
      this.alpha = generateDecimalBetween(5, 10) / 10; /* обозначение св-ва alfa от 5 до 10 и / на 10, т.к. alpha-прозрачность, которая от 0 до 1. */
...
class CanvasBackground {
   ...
   animate() {
      this.ctx.clearRect(0, 0, this.canvas.clientWidth, this.canvas.clientHeight); /*Очистить холст от точки с коорд. 0,0 до точки ширины и высоты холста*/
      ...
      this.bubblesList.forEach((bubble) => { /*Для каждого элемента класса Bubble Вычислим новую позицию пузырька */
         ...
         this.ctx.fillStyle = "rgba(" + bubble.color + "," + bubble.alpha + ")"; /*  закрасьте круг нужным цветом*/
